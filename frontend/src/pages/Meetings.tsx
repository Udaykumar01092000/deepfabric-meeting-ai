                        ...
                        return (
                            <div key={m.id} className="glass-panel animate-fade-in meeting-card">
                                ...
                                <div className="card-actions">
                                    <button onClick={() => { setSelectedTranscript(m.raw_content); setSelectedTitle(m.title); }} className="card-action-btn secondary" type="button">
                                        📄 Transcript
                                    </button>

                                    <Link to={`/meetings/${m.id}`} className="card-action-btn primary" style={{ textDecoration: "none" }}>
                                        📊 Details
                                    </Link>

                                    <button onClick={() => handleExtract(m.id)} disabled={extracting === m.id} className={`card-action-btn accent ${extracting === m.id ? "disabled" : ""}`} style={{ opacity: extracting === m.id ? 0.8 : 1 }} type="button">
                                        {extracting === m.id ? "⏳..." : "🤖 Extract"}
                                    </button>

                                    <button onClick={() => handleDelete(m.id)} className="card-action-btn danger" type="button" aria-label="Delete meeting">
